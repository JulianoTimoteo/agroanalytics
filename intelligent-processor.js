// intelligent-processor.js - VERSÃO FINAL CORRIGIDA (V.2026 - FIX 430K ERROR)
class IntelligentProcessor {
    ...
    identifyFileTypeIntelligently(matrix, fileName) {
        ...
        // Identificação do Acumulado Safra
    ...
    // Processador AcmSafra - CORRIGIDO PARA IGNORAR TOTAIS E FORÇAR NÚMEROS
    processAcmSafraData(worksheet, headerRow) {
        ...
        structuredData.forEach(row => {
            ...
            // CORREÇÃO CRÍTICA: Verificar se a linha é um "Total"
            // Se qualquer valor da linha contiver "TOTAL" ou "GERAL", ignoramos.
            const isTotalRow = Object.values(rawNormalized).some(val => 
                String(val).toUpperCase().includes('TOTAL') || 
                String(val).toUpperCase().includes('GERAL') ||
                String(val).toUpperCase().includes('SOMA')
            );

            if (isTotalRow) return; // Pula esta iteração, não adiciona ao array final
            ...
            Object.keys(rawNormalized).forEach(key => {
                ...
                // CORREÇÃO: Forçar conversão para número nas colunas críticas
            ...
            // Só adiciona se tiver pelo menos um peso ou viagem válido
            if (normalized.pesoLiquido > 0 || normalized.qtdViagem > 0) {
    ...
    processProductionData(worksheet, headerRow) {
        ...
        structuredData.forEach(row => {
            ...
            // Verificação Preliminar de Totais
            if (Object.values(normalizedRow).some(v => String(v).toUpperCase().includes('TOTAL') && isNaN(this.parseNumber(v)))) {
    ...
    parseNumber(value) {
        ...
        if (typeof value === 'string') {
            // Suporte para formatos BR (1.000,00) se necessário, mas mantendo compatibilidade com padrão americano
    ...
    parseDateTime(val) {
        ...
        if (!dateObj && typeof val === 'string') {
            ...
            // Caso 2c: Data e Hora completa CSV/SQL
            else {
